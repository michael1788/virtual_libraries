...
def combined_plot(embedding, lim_src, lim_tgt, lim_start, lim_end, e_end,
                  contour_c='#444444', m_data="o", m_gen="o", 
                  s_data=80, s_e=60, alpha_gen=1.00, alpha_data=0.85,
    ...
    plt.scatter(embedding[:lim_src, 0], embedding[:lim_src, 1], 
                lw=0, c=pal['source'], label='Source space', alpha=alpha_data, s=s_data,
                marker=m_data)
    plt.scatter(embedding[lim_src:lim_tgt, 0], embedding[lim_src:lim_tgt, 1], 
                lw=0, c=pal['target'], label='Target space', alpha=alpha_data, s=s_data,
                marker=m_data)
    
    plt.scatter(embedding[lim_tgt:lim_start, 0], embedding[lim_tgt:lim_start, 1], 
                lw=0, c=pal['e_start'], label='Epoch 0', alpha=alpha_gen, s=s_e,
                marker=m_gen, edgecolors=contour_c, linewidth=linewidth_gen)
    plt.scatter(embedding[lim_start:lim_end, 0], embedding[lim_start:lim_end, 1], 
                lw=0, c=pal['e_end'], label=f'Last epoch', alpha=alpha_gen, s=s_e,
                marker=m_gen, edgecolors=contour_c, linewidth=linewidth_gen)
    
    plt.scatter(embedding[lim_end:, 0], embedding[lim_end:, 1], 
                lw=0, c='#5A5A5A', label='Target set', alpha=1.0, s=185, 
                marker=m_data, edgecolors='k', linewidth='3')
...
if __name__ == '__main__':
    ...
    if check_epoch==40 and check_period==10:
        ...
        if verbose: print(f'Embedding shapes, common: {common_emb.shape}, fine-tuning: {ft_emb.shape}')
        
        fig_S = combined_plot(np.concatenate([common_emb, ft_emb]), 
                              lim_src, lim_tgt, lim_start, lim_end, e_end,
                              legend=True)
        plt.savefig(f'{save_path}umap_{temp}.png', dpi=150)
        
        end = time.time()
        if verbose: print(f'UMAP PROJECTION DONE in {end - start:.04} seconds')
