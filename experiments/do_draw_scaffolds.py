...
parser = argparse.ArgumentParser(description='Draw scaffolds from their smile representation')
...
def sdi(data, scaled=True):
    ...
    if scaled:
        ...
        return -sum(p(n, N) for n in data.values() if n is not 0)/math.log2(N_scaffolds)
    else: 
...
if __name__ == '__main__':
    ...
    if verbose: print(f'DRAWING SCAFFOLDS FOR {scaffolds_type} DONE in {end - start:.04} seconds')
    ####################################
